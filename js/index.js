...
(() => {
  ...
  var require_whitespace_control = __commonJS({
    "node_modules/handlebars/dist/cjs/handlebars/compiler/whitespace-control.js"(exports, module) {
      ...
      function isPrevWhitespace(body, i, isRoot) {
        ...
        var prev3 = body[i - 1], sibling = body[i - 2];
        if (!prev3) {
          return isRoot;
        }
        if (prev3.type === "ContentStatement") {
          return (sibling || !isRoot ? /\r?\n\s*?$/ : /(^|\r?\n)\s*?$/).test(prev3.original);
        }
      }
      function isNextWhitespace(body, i, isRoot) {
        ...
        var next3 = body[i + 1], sibling = body[i + 2];
        if (!next3) {
          return isRoot;
        }
        if (next3.type === "ContentStatement") {
  ...
  var require_helpers2 = __commonJS({
    "node_modules/handlebars/dist/cjs/handlebars/compiler/helpers.js"(exports) {
      ...
      var _exception = require_exception();
      var _exception2 = _interopRequireDefault(_exception);
      function validateClose(open2, close2) {
        close2 = close2.path ? close2.path.original : close2;
        if (open2.path.original !== close2) {
          var errorNode = { loc: open2.path.loc };
          throw new _exception2["default"](open2.path.original + " doesn't match " + close2, errorNode);
        }
      }
      ...
      function stripFlags(open2, close2) {
        return {
          open: open2.charAt(2) === "~",
      ...
      function preparePartialBlock(open2, program, close2, locInfo) {
        validateClose(open2, close2);
        return {
  ...
  // src/photoloader.ts
  async function loadPicture(id2) {
    const url = `${API_ROOT}/photos/${id2}`;
  ...
  async function loadResource(uri) {
    ...
    const response = await fetch(url, { credentials: "include" });
    if (!response.ok)
      throw new Error(`Erreur HTTP ${response.status} pour ${uri}`);
    return response.json();
  }
  ...
  function displayCategorie(categorie) {
    document.getElementById("la_categorie").textContent = `categorie : ${categorie.nom}`;
  }
  function displayComments(comments) {
    const ul = document.getElementById("les_commentaires");
    ul.innerHTML = comments.map((c) => `<li>(${c.pseudo}) ${c.content}</li>`).join("");
  }
  ...
  async function loadPage(href) {
    const data = await loadResource(href);
    currentGallery = data;
    return data;
  }
  async function next() {
    if (!currentGallery?.links.next)
      throw new Error("Pas de page suivante");
    return loadPage(currentGallery.links.next.href);
  }
  async function prev() {
    if (!currentGallery?.links.prev)
      throw new Error("Pas de page pr\xE9c\xE9dente");
    return loadPage(currentGallery.links.prev.href);
  }
  async function first() {
    if (!currentGallery?.links.first)
      throw new Error("Pas de premi\xE8re page");
    return loadPage(currentGallery.links.first.href);
  }
  async function last() {
    if (!currentGallery?.links.last)
      throw new Error("Pas de derni\xE8re page");
  ...
  function display_galerie(gallery) {
    ...
    container.innerHTML = gallery.photos.map((item) => `
  ...
  `;
    ...
    document.getElementById("lightbox-next").disabled = currentIndex === photos.length - 1;
  }
  function open(photoId, galleryPhotos) {
  ...
  async function getPicture(id2) {
    try {
      const data = await loadPicture(id2);
      displayPicture(data.photo);
      const categorieData = await getCategorie(data);
      displayCategorie(categorieData.categorie);
      const commentsData = await loadResource(data.links.comments.href);
      displayComments(commentsData.comments);
    } catch (error) {
      console.error("Erreur :", error);
    }
  }
  async function loadAndDisplay(action) {
    try {
  ...
  document.getElementById("les_vignettes").addEventListener("click", (event) => {
    const figure = event.target.closest("[data-photoId]");
    if (!figure)
      return;
    const photoId = Number(figure.getAttribute("data-photoId"));
    if (photoId)
      open(photoId, getCurrentPhotos());
  });
  document.getElementById("lightbox-close").addEventListener("click", () => close());
  document.getElementById("lightbox-prev").addEventListener("click", () => prev2());
  document.getElementById("lightbox-next").addEventListener("click", () => next2());
  document.getElementById("lightbox-details").addEventListener("click", async () => {
    const id2 = getCurrentId();
    close();
    await getPicture(id2);
